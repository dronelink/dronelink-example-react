...
class FuncVersions extends Component {
    state = {
        versions: null,
        more: null
    }
    ...
    componentDidMount() {
        this.unsubscribeFuncVersions = this.props.funcVersionsRef.orderBy("created", "desc").onSnapshot(snapshot => {
            ...
            this.setState({
                versions: snapshot.docs
                    .map((doc, index) => {
                        ...
                        version.ref = doc.ref
                        version.func = Dronelink.Serialization.read(version.content)
                        version.index = index
    ...
    componentWillUnmount() {
        if (this.unsubscribeFuncVersions) {
            this.unsubscribeFuncVersions()
        }
    }
            ...
            actions: [
                {
                    ...
                    color: "secondary",
                    onClick: () => {
                        this.props.firebase.deleteFuncVersion(version.ref).catch(e => {
                            ...
                            {versions.map(version => {
                                return (
                                    ...
                                    >
                                        ...
                                        <ListItemText secondary={version.func.descriptors.toString()} primary={<Moment format="MMM D, YYYY [at] h:mm:ss a" date={version.created.toDate()} />} />
...
export default compose(withTheme, withStyles(styles), withFirebase)(FuncVersions)
