...
class ComponentVersions extends Component {
    state = {
        versions: null,
        more: null
    }
    ...
    componentDidMount() {
        this.unsubscribeComponentVersions = this.props.componentVersionsRef.orderBy("created", "desc").onSnapshot(snapshot => {
            ...
            this.setState({
                versions: snapshot.docs
                    .map((doc, index) => {
                        ...
                        version.ref = doc.ref
                        version.component = Dronelink.Serialization.read(version.content)
                        version.index = index
    ...
    componentWillUnmount() {
        if (this.unsubscribeComponentVersions) {
            this.unsubscribeComponentVersions()
        }
    }
            ...
            actions: [
                {
                    ...
                    color: "secondary",
                    onClick: () => {
                        this.props.firebase.deleteComponentVersion(version.ref).catch(e => {
                            ...
                            {versions.map(version => {
                                return (
                                    ...
                                    >
                                        ...
                                        <ListItemText secondary={version.component.descriptors.toString()} primary={<Moment format="MMM D, YYYY [at] h:mm:ss a" date={version.created.toDate()} />} />
...
export default compose(withTheme, withStyles(styles), withFirebase)(ComponentVersions)
